...
impl ComCrateBuilder {
    pub fn build(self) -> ParseResult<ComCrate> {
        ...
        let interfaces_by_variants = {
            IndexMap::from_iter(self.interfaces.iter().flat_map(|itf| {
                itf.variants()
                    .iter()
                    .map(|(_, itf_variant)| {
                        (
...
impl ComCrate {
    pub fn parse(crate_name: &str, sources: &[&str]) -> ParseResult<ComCrate> {
        let mut builder: ComCrateBuilder = Default::default();

        for src in sources {
            let krate: ::syn::File = ::syn::parse_str(src)
                .map_err(|_| ParseError::ComCrate("Failed to parse source".into()))?;

            Self::process_crate_items(crate_name, None, &krate.items, &mut builder)?;
        }
    ...
    pub fn parse_package(crate_path: &Path) -> ParseResult<ComCrate> {
    ...
    pub fn parse_cargo_toml(toml_path: &Path) -> ParseResult<ComCrate> {
        let mut f = fs::File::open(toml_path)
            .map_err(|_| ParseError::CargoToml("Could not open Cargo toml".into()))?;
        let mut buf = String::new();
        f.read_to_string(&mut buf)
            .map_err(|_| ParseError::CargoToml("Could not read Cargo toml".into()))?;

        let toml = buf
            .parse::<toml::Value>()
            .map_err(|_| ParseError::CargoToml("Could not parse Cargo toml".into()))?;
        let root = match toml {
            toml::Value::Table(root) => root,
            _ => return Err(ParseError::CargoToml("Invalid TOML root element".into())),
        };

        let lib_name = match root.get("package") {
            Some(&toml::Value::Table(ref package)) => match package.get("name") {
                Some(&toml::Value::String(ref name)) => name,
                _ => {
                    return Err(ParseError::CargoToml(
        ...
        let rel_lib_path = PathBuf::from(&match root.get("lib") {
            Some(&toml::Value::Table(ref package)) => match package.get("path") {
                Some(&toml::Value::String(ref path)) => path.clone(),
    ...
    pub fn parse_file(crate_name: &str, path: &Path) -> ParseResult<ComCrate> {
        let mut builder: ComCrateBuilder = Default::default();
    ...
    fn parse_file_internal(
        ...
        let mut f = fs::File::open(path).map_err(|_| {
            ParseError::ComCrate(format!("Could not open file {}", path.to_string_lossy()))
        })?;

        let mut buf = String::new();
        f.read_to_string(&mut buf).map_err(|_| {
            ParseError::ComCrate(format!("Could not read file {}", path.to_string_lossy()))
        })?;

        let krate = ::syn::parse_file(&buf).map_err(|_| {
            ParseError::ComCrate(format!("Failed to parse source {}", path.to_string_lossy()))
        })?;
    ...
    fn process_crate_items(
        ...
        for item in items {
            let mod_item = if let ::syn::Item::Mod(ref m) = *item {
                m
            } else {
                continue;
            };
            ...
            match mod_item.content {
                None => {
                    ...
                    let mut mod_paths = vec![
                        path.parent()
                            .unwrap()
                            .join(format!("{}.rs", mod_item.ident)),
                        path.parent()
                            .unwrap()
                            .join(format!("{}/mod.rs", mod_item.ident)),
                    ]
                    .into_iter()
                    .filter(|p| p.exists());

                    let mod_path = mod_paths.next().ok_or_else(|| {
                        ParseError::ComCrate(format!("Could not find mod {}", mod_item.ident))
                    })?;

                    let more = mod_paths.next();
                    if more.is_some() {
                        return Err(ParseError::ComCrate(format!(
                            "Ambiguous mod, both {0}.rs and \
                             {0}/mod.rs present",
                            mod_item.ident
                        )));
                    }
        ...
        for item in items {
            ...
            if let ::syn::Item::Macro(m) = item {
                if let Some(ident) = m.mac.path.get_ident() {
                    if ident == "com_library" {
                        b.libs
                            .push(ComLibrary::parse(crate_name, m.mac.tokens.clone())?);
                    }
                }
            }
            ...
            for attr in &item.get_attributes().unwrap() {
                match attr.path.get_ident().unwrap().to_string().as_ref() {
                    "com_interface" => b.interfaces.push(ComInterface::from_ast(
                        crate_name,
                        attr.tokens.clone(),
                        item,
                    )?),
                    "com_class" => {
                        if let ::syn::Item::Struct(ref s) = *item {
                            b.structs
                                .push(ComStruct::from_ast(crate_name, attr.tokens.clone(), s)?)
                        } else {
                    ...
                    "com_impl" => b.impls.push(ComImpl::from_ast(item)?),
                    _ => {}
                }
            }
        }
    ...
    pub fn lib(&self) -> &Option<ComLibrary> {
        &self.lib
    }
    pub fn interfaces(&self) -> &IndexMap<String, ComInterface> {
        &self.interfaces
    }
    pub fn structs(&self) -> &IndexMap<String, ComStruct> {
        &self.structs
    }
    pub fn impls(&self) -> &Vec<ComImpl> {
        &self.impls
    }
