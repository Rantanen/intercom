...
impl<T1: Parse, T2: Parse> Parse for OneOf2<T1, T2>
{
    fn parse(input: ParseStream) -> Result<Self>
    {
        use syn::parse::discouraged::Speculative;
        let fork = input.fork();
        if let Ok(r) = fork.parse::<T1>() {
            input.advance_to(&fork);
            return Ok(OneOf2::A(r));
        }

        let fork = input.fork();
        if let Ok(r) = fork.parse::<T2>() {
            input.advance_to(&fork);
            return Ok(OneOf2::B(r));
        }
...
macro_rules! intercom_attribute {
    ...
    ( $attr:ident < $attr_param:ident, $params:ident > { $( $name:ident : $type:ty, )* } ) => {
