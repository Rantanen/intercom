...
pub fn parse<'a, 'b: 'a>(ty: &'b syn::Type) -> Option<TypeInfo<'a>> {
...
impl<'s, 'p: 's> TypeInfo<'s> {
    ...
    pub fn get_leaf(&self) -> &TypeInfo<'s> {
        match self.rust_type {
    ...
    fn from_resolver(resolver: TypeInfoResolver<'p>, original: &'p syn::Type) -> TypeInfo<'s> {
...
impl<'s> std::fmt::Display for RustType<'s> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match *self {
            ...
            RustType::Wrapper(wrapper, ref wrapped) => {
                write!(f, "{}<{}>", wrapper, wrapped.rust_type)
            }
        }
    }
}
...
impl<'s, 'p: 's> TypeInfoResolver<'s> {
    ...
    fn from_type(syn_type: &'p syn::Type) -> Option<TypeInfoResolver<'s>> {
        match *syn_type {
    ...
    fn new(rust_type: RustType<'s>) -> TypeInfoResolver<'s> {
        TypeInfoResolver {
            rust_type,
            pass_by: None,
            is_mutable: None,
    ...
    fn pass_by(resolver: TypeInfoResolver<'s>, pass_by: PassBy) -> TypeInfoResolver<'s> {
    ...
    fn mutable(resolver: TypeInfoResolver<'s>, is_mutable: bool) -> TypeInfoResolver<'s> {
    ...
    fn array(resolver: TypeInfoResolver<'s>, array_length: &'p syn::Expr) -> TypeInfoResolver<'s> {
    ...
    fn wrapped(
        ...
        let nested_type =
            TypeInfo::from_resolver(TypeInfoResolver::from_type(nested_type)?, nested_type);

        Some(TypeInfoResolver {
            rust_type: RustType::Wrapper(resolver.get_ident_for_wrapping(), Rc::new(nested_type)),
            pass_by: resolver.pass_by,
            is_mutable: resolver.is_mutable,
            array_length: resolver.array_length,
        })
    ...
    fn from_array(array: &'p syn::TypeArray) -> Option<TypeInfoResolver<'s>> {
        let resolver = TypeInfoResolver::from_type(&array.elem)?;

        Some(TypeInfoResolver::array(resolver, &array.len))
    }

    fn from_slice(slice: &'p syn::TypeSlice) -> Option<TypeInfoResolver<'s>> {
        TypeInfoResolver::from_type(&slice.elem)
    }

    fn from_reference(reference: &'p syn::TypeReference) -> Option<TypeInfoResolver<'s>> {
        let resolver = TypeInfoResolver::from_type(&reference.elem)?;
        let resolver =
            TypeInfoResolver::mutable(resolver, TypeInfoResolver::is_mutable(reference.mutability));

        Some(TypeInfoResolver::pass_by(resolver, PassBy::Reference))
    }

    fn from_pointer(ptr: &'p syn::TypePtr) -> Option<TypeInfoResolver<'s>> {
        let resolver = TypeInfoResolver::from_type(&ptr.elem)?;
        let resolver =
            TypeInfoResolver::mutable(resolver, TypeInfoResolver::is_mutable(ptr.mutability));

        Some(TypeInfoResolver::pass_by(resolver, PassBy::Ptr))
    }

    fn from_path(type_path: &'p syn::TypePath) -> Option<TypeInfoResolver<'s>> {
        TypeInfoResolver::from_segment(type_path.path.segments.last().unwrap())
    }

    fn from_segment(segment: &'p syn::PathSegment) -> Option<TypeInfoResolver<'s>> {
        ...
        let args = match segment.arguments {
            ...
            syn::PathArguments::AngleBracketed(ref data) => Some(&data.args),
            ...
            syn::PathArguments::Parenthesized(..) => panic!("Fn-types are unsupported."),
        };

        match rust_type.as_str() {
            // Extract a wrapped type.
            "ComRc" | "ComItf" | "ComResult" | "InterfacePtr" => TypeInfoResolver::wrapped(
    ...
    fn from_trait_object(trait_object: &'p syn::TypeTraitObject) -> Option<TypeInfoResolver<'s>> {
        // Find the first actual trait. Fro example lifetime parameters are ignored.
        let trait_bound = trait_object
            .bounds
            .iter()
            .find_map(|parameter: &TypeParamBound| {
                if let syn::TypeParamBound::Trait(ref tr) = parameter {
                    Some(tr)
                } else {
                    None
                }
    ...
    fn is_mutable(mutability: Option<syn::token::Mut>) -> bool {
        mutability.is_some()
    }

    /// Gets the identifier used to wrap another type.
    fn get_ident_for_wrapping(&self) -> &'s syn::Ident {
        match self.rust_type {
