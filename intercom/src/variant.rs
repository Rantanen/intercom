    ...
    unsafe fn intercom_from(src: Variant) -> ComResult<Self> {
        Ok(match src {
            ...
            Variant::String(data) => raw::Variant::new(
                ...
                raw::VariantData {
                    bstrVal: crate::BString::com_from(data)?.into_ptr(),
                },
            ),
...
impl TryFrom<Variant> for String {
    ...
    fn try_from(src: Variant) -> Result<String, Self::Error> {
        match src {
            Variant::String(data) => String::com_from(data).map_err(|_| VariantError),
            _ => Err(VariantError::from(&src)),
        }
    }
}
...
impl TryFrom<Variant> for BString {
    ...
    fn try_from(src: Variant) -> Result<BString, Self::Error> {
        match src {
            Variant::String(data) => BString::com_from(data).map_err(|_| VariantError),
            _ => Err(VariantError::from(&src)),
        }
    }
}
...
impl TryFrom<Variant> for CString {
    ...
    fn try_from(src: Variant) -> Result<CString, Self::Error> {
        match src {
            Variant::String(data) => CString::com_from(data).map_err(|_| VariantError),
            _ => Err(VariantError::from(&src)),
        }
    }
}
