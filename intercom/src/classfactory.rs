...
pub struct ClassFactoryVtbl {
    ...
    pub create_instance:
        unsafe extern "stdcall" fn(RawComPtr, RawComPtr, REFIID, *mut RawComPtr) -> raw::HRESULT,
...
pub struct ClassFactoryVtbl {
    pub __base: IUnknownVtbl,
    pub create_instance:
        unsafe extern "C" fn(RawComPtr, RawComPtr, REFIID, *mut RawComPtr) -> raw::HRESULT,
...
impl<T: Fn(REFCLSID) -> RawComResult<RawComPtr>> ClassFactory<T> {
    ...
    /// The pointers _must_ be valid.
    #[cfg(windows)]
    pub unsafe extern "stdcall" fn lock_server(self_vtbl: RawComPtr, lock: bool) -> raw::HRESULT {
    ...
    /// The pointers _must_ be valid.
    #[cfg(not(windows))]
    pub unsafe extern "C" fn lock_server(self_vtbl: RawComPtr, lock: bool) -> raw::HRESULT {
        Self::lock_server_agnostic(self_vtbl, lock)
    }
    ...
    ///
    /// The pointers _must_ be valid.
    unsafe fn lock_server_agnostic(self_vtbl: RawComPtr, lock: bool) -> raw::HRESULT {
        if lock {
