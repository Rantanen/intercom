...
pub trait IntercomFromRef<TSource> : Sized {
    fn intercom_from_ref( source : &TSource ) -> ComResult<Self>;
}

impl<TSource: Clone> IntercomFromRef<TSource> for TSource {
    fn intercom_from_ref( source: &TSource ) -> ComResult<Self> {
        Ok( source.clone() )
    }
}
...
pub trait IntercomRefInto<TTarget> : Sized {
    fn intercom_ref_into( &self ) -> ComResult<TTarget>;
}

impl<TSource, TTarget: IntercomFromRef<TSource>>
        IntercomRefInto<TTarget> for TSource
{
    fn intercom_ref_into( &self ) -> ComResult<TTarget> {
        TTarget::intercom_from_ref( self )
    }
}
