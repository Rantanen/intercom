...
fn main()
{
    ...
    let app = App::new("Rust COM utility")
        ...
        .subcommand(
            SubCommand::with_name("read-typelib")
        ...
        .subcommand(
            SubCommand::with_name("cpp")
                ...
                .arg(Arg::with_name("all").long("all").help(
                    "Include both Automation and Raw type systems in the C++ implementation.{n}\
    ...
    let app = app.subcommand(
        SubCommand::with_name("embed-typelib")
            .about("Builds and embeds the typelib into a DLL")
            .arg(Arg::with_name("path").help("Path to the DLL.").index(1)),
    );
...
fn run_cmd(matches: &ArgMatches) -> Result<(), failure::Error>
{
    ...
    match matches.subcommand() {
        ...
        ("cpp", Some(args)) => {
            ...
            let header_writer: Result<_, failure::Error> = args
                .value_of("header")
                .map(|path| {
                    if path == "-" {
            ...
            let source_writer: Result<_, failure::Error> = args
                .value_of("source")
                .map(|path| {
                    if path == "-" {
                        Ok(Box::new(io::stdout()) as Box<dyn io::Write>)
